...
func Parse(data []byte) (vault.Env, error) {
...
func (p *envParser) consume(l line) error {
	...
	p.keys[key] = struct{}{}
	p.env.Vars = append(p.env.Vars, vault.Var{Key: key, Value: value})
	return nil
}
...
func (p *envParser) metadata(l line) error {
	...
	case strings.HasPrefix(body, descriptionMarker):
		...
		p.description = true
		p.env.Description = strings.TrimSpace(body[len(descriptionMarker):])
...
func parseTags(raw string) ([]string, error) {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err := vault.ValidateTag(tag); err != nil {
			return nil, err
		}
