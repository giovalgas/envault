...
func (f *templateFlags) options() merge.Options {
	return merge.Options{OnlyTemplate: f.only}
}
...
func templateConvert(parsed dotenv.Template) merge.Template {
	entries := make([]merge.TemplateEntry, len(parsed.Entries))
	for i, entry := range parsed.Entries {
		entries[i] = merge.TemplateEntry{Key: entry.Key, Default: entry.Default, HasDefault: entry.HasDefault}
	}
	return merge.Template{Entries: entries}
}
