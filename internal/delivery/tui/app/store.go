...
func (s VaultStore) List(ctx context.Context) ([]vaultusecase.EnvView, error) {
	return s.ListEnvs.Execute(ctx, vaultusecase.ListEnvsQuery{})
}

func (s VaultStore) Get(ctx context.Context, name string) (vaultusecase.EnvView, error) {
	return s.ShowEnv.Execute(ctx, name)
}

func (s VaultStore) Copy(ctx context.Context, src, dst string) (vaultusecase.EnvView, error) {
	return s.CopyEnv.Execute(ctx, src, dst)
}

func (s VaultStore) Rename(ctx context.Context, oldName, newName string) (vaultusecase.EnvView, error) {
