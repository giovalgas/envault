...
func runAsBinary() int {
	wiring := Wiring{Vault: binaryVault, KeyMigration: testKeyMigration, Compose: testCompose, Skill: testSkill}
	return Execute(context.Background(), NewApp("test", wiring), os.Args[1:])
}

func binaryVault(cfg config.Config, _ Streams) (VaultUseCases, error) {
	keys, err := keystore.WithOverride(cfg.KeyOverride, keystore.NewFile(cfg.Paths.Key))
	if err != nil {
		return VaultUseCases{}, err
	}
	return NewVaultUseCases(VaultDeps{Repository: encryptedfile.New(cfg.Paths, keys)}), nil
}
...
func TestShellInitPrintsWrapper(t *testing.T) {
	for _, dialect := range shellDialects {
		ta := newTestApp(t)
		if code := ta.run("shell-init", dialect); code != ExitOK {
...
func TestLoadWithoutWrapperExitsUsage(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a", "b"); code != ExitUsage {
	...
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a", "--json"); code != ExitUsage {
		t.Fatalf("json code = %d", code)
	}
	if envelope := decodeEnvelope(t, ta.Out.Bytes()); envelope.Error.Code != ExitUsage || !strings.Contains(envelope.Error.Message, "shell-init") {
...
func TestLoadWithoutWrapperSuggestsFishSyntax(t *testing.T) {
	planTestWorkdir(t)
	shellInitSetExport(t, "", "")
	t.Setenv(shellEnvVar, "/usr/local/bin/fish")
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a"); code != ExitUsage {
...
func TestLoadShellWritesExportFile(t *testing.T) {
	cases := []struct {
		...
		{dialect: shellBash, want: "export X='2'\nexport DATABASE_URL='" + planTestSecretDB + "'\nexport APP_URL='" + planTestSecretApp + "'\n"},
...
func TestLoadShellDialectFallsBackToShellEnv(t *testing.T) {
	...
	t.Setenv(shellEnvVar, "/usr/bin/fish")
	ta := newTestApp(t)
	ta.seed(t, planTestEnv("a", "X", "1"))
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a"); code != ExitOK {
		t.Fatalf("code = %d stderr %s", code, ta.Err.String())
	}
...
func TestLoadShellJSONAndTemplate(t *testing.T) {
	...
	exports := filepath.Join(t.TempDir(), "exports")
	shellInitSetExport(t, exports, shellZsh)
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a", "--only-template", "--json"); code != ExitOK {
		t.Fatalf("code = %d stderr %s", code, ta.Err.String())
	}
	...
	if out.Mode != loadModeExported || out.Target.Mode != targetModeShell || out.Target.Path != "" {
		t.Fatalf("out = %+v", out)
	}
...
func TestLoadShellErrors(t *testing.T) {
	planTestWorkdir(t)
	shellInitSetExport(t, t.TempDir(), shellZsh)
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a"); code != ExitValidation {
		t.Fatalf("directory code = %d", code)
	}
	for _, args := range [][]string{{"load", "a", "--force"}, {"load", "a", "--merge"}, {"load", "a", "--out", ""}} {
		if code := ta.runWith(newLoadCmd(ta.App), args...); code != ExitUsage {
			t.Fatalf("%v: code = %d", args, code)
		}
	}
...
func TestLoadOutIgnoresWrapper(t *testing.T) {
	...
	if code := ta.runWith(newLoadCmd(ta.App), "load", "a", "--out", ".env"); code != ExitOK {
		t.Fatalf("code = %d stderr %s", code, ta.Err.String())
	}
...
func TestPlanWithoutOutReportsShellTarget(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.runWith(newPlanCmd(ta.App), "plan", "a", "b"); code != ExitOK {
	...
	out := planTestDecode(t, ta.Out.Bytes())
	if out.Target.Mode != targetModeShell || out.Target.Path != "" || out.Target.Exists {
	...
	planTestNoSecrets(t, ta)
	if code := ta.runWith(newPlanCmd(ta.App), "plan", "a", "--out", ""); code != ExitUsage {
