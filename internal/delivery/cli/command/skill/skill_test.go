...
func TestSkillInstallCustomDir(t *testing.T) {
	...
	if code := ta.Run("skill", "install", "--dir", dir); code != presenter.ExitOK {
...
func TestSkillInstallDefaultDir(t *testing.T) {
	...
	if code := ta.Run("skill", "install"); code != presenter.ExitOK {
		t.Fatalf("code = %d (%s)", code, ta.Err.String())
	}
...
func TestSkillUsageErrors(t *testing.T) {
	ta := newTestApp(t)
	if code := ta.Run("skill", "install", "extra"); code != presenter.ExitUsage {
		t.Fatalf("code = %d, want %d", code, presenter.ExitUsage)
	}
	if code := ta.Run("skill", "install", "--nope"); code != presenter.ExitUsage {
		t.Fatalf("code = %d, want %d", code, presenter.ExitUsage)
	}
}
...
func TestSkillWithoutSubcommandShowsHelp(t *testing.T) {
	ta := newTestApp(t)
	if code := ta.Run("skill"); code != presenter.ExitOK {
