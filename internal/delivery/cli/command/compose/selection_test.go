...
func (f *tuiSelecting) run(ctx context.Context, session app.TUISession) error {
...
func (ta *testApp) selectionJSON(t *testing.T) (presenter.SelectionEnvelope, map[string]json.RawMessage) {
	t.Helper()
	var envelope presenter.SelectionEnvelope
...
func TestSelectionMadeByTUI(t *testing.T) {
	...
	if code := ta.Run(); code != presenter.ExitOK || fake.err != nil {
		t.Fatalf("TUI: code = %d err = %v stderr = %q", code, fake.err, ta.Err.String())
	}
	ta.SetTerminal(false, false)

	if code := ta.Run("selection", "--json"); code != presenter.ExitOK {
		t.Fatalf("code = %d stderr = %q", code, ta.Err.String())
	}
	envelope, raw := ta.selectionJSON(t)
	if envelope.SchemaVersion != presenter.SchemaVersion || !slices.Equal(envelope.Envs, []string{"b", "a"}) || len(envelope.Missing) != 0 {
...
func TestSelectionReportsMissing(t *testing.T) {
	...
	if code := ta.Run("selection"); code != presenter.ExitOK {
		t.Fatalf("code = %d stderr = %q", code, ta.Err.String())
	}
	if ta.Out.String() != "b\na\n" {
...
func TestSelectionAfterCLIRename(t *testing.T) {
	...
	if code := ta.Run("rename", "a", "c"); code != presenter.ExitOK {
...
func TestSelectionNotInitialized(t *testing.T) {
	for _, args := range [][]string{{"selection"}, {"selection", "--json"}} {
		ta := newTestApp(t)
		if code := ta.Run(args...); code != presenter.ExitNotInitialized {
...
func TestSelectionRejectsArgs(t *testing.T) {
	ta := newTestApp(t)
	ta.seed(t, selectionSeed()...)
	if code := ta.Run("selection", "a"); code != presenter.ExitUsage {
