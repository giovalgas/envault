...
func TestShellDialects(t *testing.T) {
	...
	fish := "set -gx X '2'\n" +
		...
		"set -gx MULTI 'l1\nl2'\n" +
		"set -gx EMPTY ''\n"
	cases := map[string]string{composeusecase.ShellBash: posix, composeusecase.ShellZsh: posix, composeusecase.ShellFish: fish}
	for dialect, want := range cases {
		t.Run(dialect, func(t *testing.T) {
			planTestWorkdir(t)
			ta := newTestApp(t)
			shellTestSeed(t, ta)
			if code := ta.Run("shell", "a", "b", "--shell", dialect); code != presenter.ExitOK {
...
func TestShellDefaultDialect(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if code := ta.Run("shell", "a"); code != presenter.ExitOK {
...
func TestShellIgnoresTemplate(t *testing.T) {
	...
	ta := newTestApp(t)
	ta.seed(t, planTestEnv("a", "X", "1"))
	if code := ta.Run("shell", "a", "--shell", composeusecase.ShellBash); code != presenter.ExitOK {
...
func TestShellErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unsupported shell", args: []string{"shell", "a", "--shell", "powershell"}, code: presenter.ExitUsage},
		{name: "empty shell", args: []string{"shell", "a", "--shell", ""}, code: presenter.ExitUsage},
		{name: "no env", args: []string{"shell"}, code: presenter.ExitUsage},
		{name: "unknown env", args: []string{"shell", "nao-existe"}, code: presenter.ExitEnvNotFound},
	}
	for _, tc := range cases {
