...
func TestPlanDoesNotWriteAndReportsPlan(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.Run("plan", "a", "b", "--out", ".env"); code != presenter.ExitOK {
	...
	if out.SchemaVersion != presenter.SchemaVersion || !slices.Equal(out.Envs, []string{"a", "b"}) {
		t.Fatalf("header = %+v", out)
	}
	if out.Target.Path != ".env" || out.Target.Exists || out.Target.Gitignored != nil {
		t.Fatalf("target = %+v", out.Target)
	}
	if out.Template.Found || out.Template.Path == nil || *out.Template.Path != composeusecase.DefaultTemplateFile {
...
func TestPlanTemplateDetection(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.Run("plan", "a"); code != presenter.ExitOK {
		t.Fatalf("code = %d stdout %s", code, ta.Out.String())
	}
	out := planTestDecode(t, ta.Out.Bytes())
	if !out.Template.Found || *out.Template.Path != composeusecase.DefaultTemplateFile {
		t.Fatalf("template = %+v", out.Template)
	}
	...
	if entries := planTestEntries(t, dir); !slices.Equal(entries, []string{composeusecase.DefaultTemplateFile}) {
...
func TestPlanNoTemplate(t *testing.T) {
	dir := planTestWorkdir(t)
	planTestWrite(t, filepath.Join(dir, composeusecase.DefaultTemplateFile), "SENTRY_DSN=\n")
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.Run("plan", "a", "--no-template"); code != presenter.ExitOK {
		t.Fatalf("code = %d stdout %s", code, ta.Out.String())
	}
	out := planTestDecode(t, ta.Out.Bytes())
...
func TestPlanExplicitTemplateAndOnlyTemplate(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	code := ta.Run("plan", "a", "b", "--template", "custom.example", "--only-template")
...
func TestPlanTemplateErrors(t *testing.T) {
	cases := []struct {
		...
		{name: "explicit missing", args: []string{"--template", "nao-existe.example"}, code: presenter.ExitValidation},
		{name: "syntax error", template: "NOT VALID\n", args: nil, code: presenter.ExitValidation},
		{name: "only template without template", args: []string{"--only-template"}, code: presenter.ExitValidation},
		{name: "template with no-template", args: []string{"--template", "x", "--no-template"}, code: presenter.ExitUsage},
		{name: "only with no-template", args: []string{"--only-template", "--no-template"}, code: presenter.ExitUsage},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			dir := planTestWorkdir(t)
			if tc.template != "" {
				planTestWrite(t, filepath.Join(dir, composeusecase.DefaultTemplateFile), tc.template)
			}
			ta := newTestApp(t)
			planTestSeed(t, ta)
			args := append([]string{"plan", "a"}, tc.args...)
			if code := ta.Run(args...); code != tc.code {
				t.Fatalf("code = %d, want %d (stdout %s)", code, tc.code, ta.Out.String())
			}
			if envelope := clitest.DecodeEnvelope(t, ta.Out.Bytes()); envelope.Error.Code != tc.code {
...
func TestPlanVaultErrorsAreJSON(t *testing.T) {
	planTestWorkdir(t)
	ta := newTestApp(t)
	if code := ta.Run("plan", "a"); code != presenter.ExitNotInitialized {
	...
	planTestSeed(t, ta)
	if code := ta.Run("plan", "a", "nao-existe"); code != presenter.ExitEnvNotFound {
		t.Fatalf("code = %d", code)
	}
	if envelope := clitest.DecodeEnvelope(t, ta.Out.Bytes()); envelope.Error.Code != presenter.ExitEnvNotFound {
		t.Fatalf("envelope = %+v", envelope)
	}
	if code := ta.Run("plan", "a", "a"); code != presenter.ExitUsage {
		t.Fatalf("duplicate env code = %d", code)
	}
	if code := ta.Run("plan"); code != presenter.ExitUsage {
...
func TestPlanAcceptsJSONFlag(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := ta.Run("plan", "a", "--json"); code != presenter.ExitOK {
		t.Fatalf("code = %d", code)
	}
...
func TestPlanTargetInRepo(t *testing.T) {
	...
	if code := ta.Run("plan", "a", "--out", "exposto.env"); code != presenter.ExitOK {
		t.Fatalf("code = %d", code)
	}
	out = planTestDecode(t, ta.Out.Bytes())
	if out.Target.Exists || out.Target.Gitignored == nil || *out.Target.Gitignored {
