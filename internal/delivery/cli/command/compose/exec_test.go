...
func TestExecInjectsVariables(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := execTestRun(t, ta, []string{"-e", "a"}, "print", "X", "DATABASE_URL"); code != presenter.ExitOK {
...
func TestExecLastWinsOverProcessEnv(t *testing.T) {
	...
	for _, flags := range [][]string{{"-e", "a,b"}, {"-e", "a", "-e", "b"}, {"--env", "a,b"}} {
		if code := execTestRun(t, ta, flags, "print", "X", "APP_URL", "ENVAULT_TEST_EXEC_KEEP"); code != presenter.ExitOK {
...
func TestExecPassesStdin(t *testing.T) {
	...
	if code := execTestRun(t, ta, []string{"-e", "a"}, "stdin"); code != presenter.ExitOK {
		t.Fatalf("code = %d stderr %s", code, ta.Err.String())
	}
...
func TestExecTemplate(t *testing.T) {
	...
	ta := newTestApp(t)
	planTestSeed(t, ta)
	if code := execTestRun(t, ta, []string{"-e", "a"}, "print", "PORT", "X", "DATABASE_URL"); code != presenter.ExitOK {
		t.Fatalf("code = %d stderr %s", code, ta.Err.String())
	}
	...
	if code := execTestRun(t, ta, []string{"-e", "a", "--only-template"}, "print", "DATABASE_URL"); code != presenter.ExitOK {
		t.Fatalf("code = %d", code)
	}
	if got := ta.Out.String(); got != "<unset>" {
		t.Fatalf("only-template stdout = %q", got)
	}
	if code := execTestRun(t, ta, []string{"-e", "a", "--no-template"}, "print", "PORT"); code != presenter.ExitOK {
		t.Fatalf("code = %d", code)
	}
	if got := ta.Out.String(); got != "<unset>" {
...
func TestExecErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "without env", args: []string{"exec", "--", os.Args[0]}, code: presenter.ExitUsage},
		{name: "without command", args: []string{"exec", "-e", "a"}, code: presenter.ExitUsage},
		{name: "unknown env", args: []string{"exec", "-e", "a,nao-existe", "--", os.Args[0]}, code: presenter.ExitEnvNotFound},
		{name: "command not found", args: []string{"exec", "-e", "a", "--", filepath.Join(t.TempDir(), "nao-existe")}, code: presenter.ExitError},
	}
	for _, tc := range cases {
		if code := ta.Run(tc.args...); code != tc.code {
