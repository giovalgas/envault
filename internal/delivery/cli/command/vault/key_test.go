...
func keyTestFileKey(t *testing.T, ta *testApp) []byte {
	t.Helper()
	key, err := keystore.NewFile(filepath.Join(ta.Home, config.KeyFileName)).Load(context.Background())
	if err != nil {
...
func keyTestKeychain(t *testing.T, ta *testApp) *keystore.Keyring {
	t.Helper()
...
func TestKeyMigrateMovesKeyToKeychain(t *testing.T) {
	...
	if code := ta.Run("key", "migrate"); code != presenter.ExitOK {
	...
	if !strings.Contains(ta.Err.String(), "migrada") || strings.Contains(ta.Err.String(), keystore.EncodeKey(original)) {
...
func TestKeyMigrateKeychainUnavailable(t *testing.T) {
	...
	code := ta.Run("key", "migrate")
	if code != presenter.ExitError {
		t.Fatalf("code = %d, want %d", code, presenter.ExitError)
	}
	stderr := ta.Err.String()
	if !strings.Contains(stderr, vault.ErrKeyringUnavailable.Error()) || !strings.Contains(stderr, "mantido") {
...
func TestKeyMigrateKeychainHoldsOtherKey(t *testing.T) {
	...
	if code := ta.Run("key", "migrate"); code != presenter.ExitError {
		t.Fatalf("code = %d (%s)", code, ta.Err.String())
	}
	if !keyTestKeyFileExists(t, ta) {
		t.Fatal("arquivo key foi removido")
...
func TestKeyMigrateFileKeyDoesNotOpenVault(t *testing.T) {
	...
	if err := os.WriteFile(keyPath, []byte(keystore.EncodeKey(other)+"\n"), config.FilePerm); err != nil {
		t.Fatal(err)
	}
	if code := ta.Run("key", "migrate"); code != presenter.ExitDecrypt {
		t.Fatalf("code = %d, want %d (%s)", code, presenter.ExitDecrypt, ta.Err.String())
	}
	if !keyTestKeyFileExists(t, ta) {
		t.Fatal("arquivo key foi removido")
	}
	if _, err := keyTestKeychain(t, ta).Load(context.Background()); !errors.Is(err, vault.ErrNoKey) {
...
func TestKeyMigrateWithoutVault(t *testing.T) {
	keyring.MockInit()
	ta := newTestApp(t)
	if code := ta.Run("key", "migrate"); code != presenter.ExitNotInitialized {
		t.Fatalf("code = %d, want %d (%s)", code, presenter.ExitNotInitialized, ta.Err.String())
	}
}
...
func TestKeyMigrateKeyOnlyWithoutVaultFile(t *testing.T) {
	keyring.MockInit()
	ta := newTestApp(t)
	if _, err := keystore.NewFile(filepath.Join(ta.Home, config.KeyFileName)).Create(context.Background()); err != nil {
		t.Fatal(err)
	}
	if code := ta.Run("key", "migrate"); code != presenter.ExitOK {
		t.Fatalf("code = %d (%s)", code, ta.Err.String())
...
func TestKeyMigrateUsageError(t *testing.T) {
	ta := newTestApp(t)
	if code := ta.Run("key", "migrate", "extra"); code != presenter.ExitUsage {
		t.Fatalf("code = %d, want %d", code, presenter.ExitUsage)
	}
}

func TestKeyWithoutSubcommandShowsHelp(t *testing.T) {
	ta := newTestApp(t)
	if code := ta.Run("key"); code != presenter.ExitOK {
...
func TestKeyAccountIsAbsoluteVaultDir(t *testing.T) {
	...
	account := keystore.Account(cfg.Paths.Dir)
