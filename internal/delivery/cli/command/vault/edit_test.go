...
func TestEditUnchanged(t *testing.T) {
	...
	if code := ta.Run("edit", "a"); code != presenter.ExitOK {
...
func TestEditEmptyFileCancels(t *testing.T) {
	...
	if code := ta.Run("edit", "a"); code != presenter.ExitCanceled {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	if env := newGetEnv(t, ta); !env.SameContent(editSeedEnv()) {
		t.Fatalf("env changed: %+v", env)
...
func TestEditNotFound(t *testing.T) {
	...
	ta.initVault(t)
	script := editortest.Install(t)
	if code := ta.Run("edit", "nao-existe"); code != presenter.ExitEnvNotFound {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	if script.Calls() != 0 {
...
func TestEditEditorFailureKeepsVault(t *testing.T) {
	...
	if code := ta.Run("edit", "a"); code != presenter.ExitError {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	if env := newGetEnv(t, ta); !env.SameContent(editSeedEnv()) {
		t.Fatalf("env changed: %+v", env)
