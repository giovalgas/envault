...
func TestNewFromFileWithTags(t *testing.T) {
	...
	code := ta.Run("new", "a", "--from-file", path, "--tags", "db,local")
...
func TestNewFromFileKeepsFileMetadata(t *testing.T) {
	...
	if code := ta.Run("new", "a", "--from-file", path); code != presenter.ExitOK {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	env := newGetEnv(t, ta)
...
func TestNewFromFileDescriptionFlagWins(t *testing.T) {
	...
	ta.initVault(t)
	path := newWriteFile(t, "# @description: do arquivo\nA=1\n")
	code := ta.Run("new", "a", "--from-file", path, "--description", "da flag")
	if code != presenter.ExitOK {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
...
func TestNewFromFileSyntaxError(t *testing.T) {
	...
	code := ta.Run("new", "a", "--from-file", path)
...
func TestNewFromMissingFile(t *testing.T) {
	ta := newTestApp(t)
	ta.initVault(t)
	code := ta.Run("new", "a", "--from-file", filepath.Join(t.TempDir(), "nao-existe.env"))
...
func TestNewWithEditor(t *testing.T) {
	...
	code := ta.Run("new", "a", "--description", "banco", "--tags", "db")
	if code != presenter.ExitOK {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	seen := script.Seen(1)
...
func TestNewEditorCanceled(t *testing.T) {
	...
	if code := ta.Run("new", "a"); code != presenter.ExitCanceled {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	newAssertAbsent(t, ta, "a")
}
...
func TestNewEditorReopensThenCreates(t *testing.T) {
	...
	if code := ta.Run("new", "a"); code != presenter.ExitOK {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
...
func TestNewEditorReopenedUnchangedCancels(t *testing.T) {
	...
	if code := ta.Run("new", "nova"); code != presenter.ExitCanceled {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	if script.Calls() != 2 {
...
func TestNewExistingEnvFailsBeforeEditor(t *testing.T) {
	...
	script := editortest.Install(t)
	if code := ta.Run("new", "a"); code != presenter.ExitValidation {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	if script.Calls() != 0 {
		t.Fatalf("editor opened %d time(s)", script.Calls())
...
func TestNewInvalidInput(t *testing.T) {
	...
	for _, args := range cases {
		ta := newTestApp(t)
		ta.initVault(t)
		if code := ta.Run(args...); code != presenter.ExitValidation {
...
func TestNewNotInitialized(t *testing.T) {
	ta := newTestApp(t)
	path := newWriteFile(t, "A=1\n")
	if code := ta.Run("new", "a", "--from-file", path); code != presenter.ExitNotInitialized {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
}
...
func TestNewEditorFailure(t *testing.T) {
	...
	if code := ta.Run("new", "a"); code != presenter.ExitError {
		t.Fatalf("code = %d, stderr = %q", code, ta.Err.String())
	}
	newAssertAbsent(t, ta, "a")
}
