...
func newTestVault(t *testing.T) *vault.Vault {
	...
	keys, err := store.NewStatic(key)
	if err != nil {
		t.Fatalf("criar keystore: %v", err)
	}
	v := vault.New(config.PathsIn(t.TempDir()), keys)
	ctx := context.Background()
	if _, err := v.Init(ctx); err != nil {
		t.Fatalf("init: %v", err)
	}
	for _, env := range seedEnvs() {
		if _, err := v.Create(ctx, env); err != nil {
...
func TestAppListRendersWithoutValues(t *testing.T) {
	...
	for _, want := range []string{"postgres-local", "redis", "stripe-test", "DATABASE_URL", "PGPASSWORD", maskedValue, "Postgres local via docker-compose", "db, local", "2 chaves"} {
...
func (failingStore) List(context.Context) ([]vault.Env, error) {
...
func (s *signalingStore) List(ctx context.Context) ([]vault.Env, error) {
