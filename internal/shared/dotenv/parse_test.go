...
func vars(pairs ...string) []Var {
	out := make([]Var, 0, len(pairs)/2)
	for i := 0; i+1 < len(pairs); i += 2 {
		out = append(out, Var{Key: pairs[i], Value: pairs[i+1]})
	}
	return out
}
...
func TestParseValidFixtures(t *testing.T) {
	cases := []struct {
		...
		{
			file: "valid/crlf.env",
			want: Document{Vars: vars("CRLF", "1", "OTHER", "two")},
		},
	}
...
func TestParseScenarios(t *testing.T) {
	cases := []struct {
		...
		{"metadados", "# @description: Postgres local\n# @tags: db, local\n", Document{Description: "Postgres local", Tags: []string{"db", "local"}}},
		{"comentário comum", "# qualquer coisa\nA=1\n", Document{Vars: vars("A", "1")}},
		{"export", "export A=1", Document{Vars: vars("A", "1")}},
		{"export com tab", "export\tA=1", Document{Vars: vars("A", "1")}},
		{"chave export", "export=1", Document{Vars: vars("export", "1")}},
		{"aspas simples literais", `A='x\ny'`, Document{Vars: vars("A", `x\ny`)}},
		{"aspas duplas com escape", `A="x\ny"`, Document{Vars: vars("A", "x\ny")}},
		{"escape desconhecido preservado", `A="x\qy"`, Document{Vars: vars("A", `x\qy`)}},
		{"comentário inline", "A=abc # nota", Document{Vars: vars("A", "abc")}},
		{"hash colado", "A=abc#def", Document{Vars: vars("A", "abc#def")}},
		{"hash inicial colado", "A=#abc", Document{Vars: vars("A", "#abc")}},
		{"comentário após aspas simples", "A='x' # c", Document{Vars: vars("A", "x")}},
...
func TestIsBlank(t *testing.T) {
	blank := []string{"", "\n\n", "# a\n  # b\n", string(FormatEditor(Document{Description: "x", Tags: []string{"a"}}))}
...
func assertSameEnv(t *testing.T, got, want Document) {
	t.Helper()
