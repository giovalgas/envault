...
func FuzzParse(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, raw []byte, description, tags, keyA, valueA, keyB, valueB string) {
		...
		assertRoundTrip(t, Document{
			Description: normalizeDescription(description),
			Tags:        normalizeTags(tags),
			Vars:        []Var{{Key: first, Value: valueA}, {Key: second, Value: valueB}},
		})
	})
}

func assertRoundTrip(t *testing.T, env Document) {
	t.Helper()
	if err := validDocument(env); err != nil {
