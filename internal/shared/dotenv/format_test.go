...
func TestFormatLines(t *testing.T) {
	cases := []struct {
		...
		{"simples", Document{Vars: vars("A", "abc")}, "A=abc\n"},
		{"com espaço", Document{Vars: vars("A", "a b")}, "A=\"a b\"\n"},
		{"vazio", Document{Vars: vars("A", "")}, "A=\"\"\n"},
		{"ordem", Document{Vars: vars("B", "2", "A", "1")}, "B=2\nA=1\n"},
		{
			"metadados",
			Document{Description: "Postgres local", Tags: []string{"db", "local"}, Vars: vars("A", "1")},
			"# @description: Postgres local\n# @tags: db, local\n\nA=1\n",
		},
		{"só tags", Document{Tags: []string{"db"}}, "# @tags: db\n"},
		{"nada", Document{}, ""},
	}
	for _, tc := range cases {
...
func TestFormatEditorEmptyEnv(t *testing.T) {
	got := string(FormatEditor(Document{}))
