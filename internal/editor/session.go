...
var (
	ErrCanceled = errors.New("edição cancelada")
...
func Open(initial vault.Env, opts Options) (*Session, error) {
	...
	content := dotenv.FormatEditor(initial)
...
func (s *Session) Review(runErr error) (Result, error) {
	...
	edited.Description = parsed.Description
	edited.Tags = parsed.Tags
	edited.Vars = parsed.Vars
	...
	return Result{Env: edited, Changed: true, Diff: Compare(s.initial, edited)}, nil
}
