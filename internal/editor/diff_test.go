...
func TestEditorDiffByKey(t *testing.T) {
	before := vault.Env{Vars: []vault.Var{{Key: "A", Value: "valor-a"}, {Key: "B", Value: "valor-b-antigo"}}}
	after := vault.Env{Vars: []vault.Var{
		{Key: "A", Value: "valor-a"},
...
func TestEditorDiffRemovedAndMetadata(t *testing.T) {
	before := vault.Env{Description: "antes", Tags: []string{"db"}, Vars: []vault.Var{{Key: "A", Value: "1"}, {Key: "B", Value: "2"}}}
	after := vault.Env{Description: "depois", Tags: []string{"db"}, Vars: []vault.Var{{Key: "B", Value: "2"}}}
	d := Compare(before, after)
...
func TestEditorDiffTagsAndOrder(t *testing.T) {
	before := vault.Env{Tags: []string{"a"}, Vars: []vault.Var{{Key: "A", Value: "1"}, {Key: "B", Value: "2"}}}
	after := vault.Env{Tags: []string{"a", "b"}, Vars: []vault.Var{{Key: "B", Value: "2"}, {Key: "A", Value: "1"}}}
	d := Compare(before, after)
...
func TestEditorDiffEmpty(t *testing.T) {
	env := vault.Env{Vars: []vault.Var{{Key: "A", Value: "1"}}}
	d := Compare(env, env)
