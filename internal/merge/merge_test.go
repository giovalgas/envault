...
func env(name string, pairs ...string) vault.Env {
	e := vault.Env{Name: name}
	for i := 0; i+1 < len(pairs); i += 2 {
		e.Vars = append(e.Vars, vault.Var{Key: pairs[i], Value: pairs[i+1]})
	}
	return e
}
...
func TestMergeResolveOrigins(t *testing.T) {
	plan := Resolve([]vault.Env{env("a", "X", "1", "Y", "2"), env("b", "X", "2")}, nil, Options{})
...
func TestMergeResolveMissingIsNotResolved(t *testing.T) {
	plan := Resolve([]vault.Env{env("a", "A", "1")}, tmpl(bare("SENTRY_DSN")), Options{})
...
func TestMergeResolveIsPure(t *testing.T) {
	envs := []vault.Env{env("a", "X", "1", "DEBUG", "1"), env("b", "X", "2")}
	template := tmpl(withDefault("PORT", "3000"), bare("X"), bare("MISSING"))
	before := []vault.Env{envs[0].Clone(), envs[1].Clone()}
