...
func (p Plan) Pairs() []vault.Var {
	pairs := make([]vault.Var, len(p.Vars))
	for i, resolved := range p.Vars {
		pairs[i] = vault.Var{Key: resolved.Key, Value: resolved.Value}
	}
	return pairs
}
...
func collect(envs []vault.Env) occurrences {
...
func Resolve(envs []vault.Env, tmpl *Template, opts Options) Plan {
	occ := collect(envs)
	plan := Plan{
		Envs:      envNames(envs),
		Vars:      []Resolved{},
...
func envNames(envs []vault.Env) []string {
...
func MergeVars(existing, incoming []vault.Var) []vault.Var {
	updates := make(map[string]string, len(incoming))
	for _, v := range incoming {
		updates[v.Key] = v.Value
	}
	merged := make([]vault.Var, 0, len(existing)+len(incoming))
