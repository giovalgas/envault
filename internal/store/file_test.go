...
func TestFileLoadWithoutKey(t *testing.T) {
	...
	if _, err := fs.Load(context.Background()); !errors.Is(err, ErrNoKey) {
...
func TestFileCreateTwiceKeepsKey(t *testing.T) {
	...
	if _, err := fs.Create(context.Background()); !errors.Is(err, ErrKeyExists) {
...
func TestFileMalformed(t *testing.T) {
	...
	for name, content := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if _, err := NewFile(path).Load(context.Background()); !errors.Is(err, ErrMalformedKey) {
...
func TestFileLoadReadError(t *testing.T) {
	dir := t.TempDir()
	if _, err := NewFile(dir).Load(context.Background()); err == nil || errors.Is(err, ErrNoKey) {
...
func TestFileRemove(t *testing.T) {
	fs := NewFile(filepath.Join(t.TempDir(), "key"))
	if err := fs.Remove(context.Background()); !errors.Is(err, ErrNoKey) {
		t.Fatalf("Remove missing err = %v, want ErrNoKey", err)
	}
	...
	if _, err := fs.Load(context.Background()); !errors.Is(err, ErrNoKey) {
		t.Fatalf("Load after Remove err = %v, want ErrNoKey", err)
	}
}
...
func TestFileRemoveError(t *testing.T) {
	...
	if err := NewFile(dir).Remove(context.Background()); err == nil || errors.Is(err, ErrNoKey) {
...
func TestDecodeKey(t *testing.T) {
	...
	for _, bad := range []string{"", "@@", EncodeKey(make([]byte, 33))} {
		if _, err := DecodeKey(bad); !errors.Is(err, ErrMalformedKey) {
			t.Fatalf("DecodeKey(%q) err = %v, want ErrMalformedKey", bad, err)
		}
	}
}
...
func TestWithOverride(t *testing.T) {
	...
	if _, err := ks.Create(context.Background()); !errors.Is(err, ErrKeyExists) {
	...
	if _, err := WithOverride(EncodeKey(make([]byte, 8)), fallback); !errors.Is(err, ErrMalformedKey) {
		t.Fatalf("short override err = %v, want ErrMalformedKey", err)
	}
}
...
func TestOverrideBeatsFileKey(t *testing.T) {
	...
	override := bytes.Repeat([]byte{3}, crypto.KeySize)
...
func TestNewStaticValidatesSize(t *testing.T) {
	if _, err := NewStatic(make([]byte, 5)); !errors.Is(err, ErrMalformedKey) {
		t.Fatalf("err = %v, want ErrMalformedKey", err)
	}
}
...
func TestChain(t *testing.T) {
	...
	missing := &fakeStore{loadErr: ErrNoKey}
	present := &fakeStore{key: key}
	loaded, err := Chain{missing, present}.Load(context.Background())
	if err != nil || !bytes.Equal(loaded, key) {
		t.Fatalf("Load = %v, %v", loaded, err)
	}
	if _, err := (Chain{missing, missing}).Load(context.Background()); !errors.Is(err, ErrNoKey) {
		t.Fatalf("all missing err = %v, want ErrNoKey", err)
	}
