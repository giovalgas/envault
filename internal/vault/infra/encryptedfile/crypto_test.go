...
func TestTamperedHeaderFails(t *testing.T) {
	...
	if _, err := Open(key, tamperedVersion); !errors.Is(err, domain.ErrDecrypt) {
		t.Fatalf("version tamper err = %v, want domain.ErrDecrypt", err)
	}
	tamperedMagic := bytes.Clone(sealed)
	tamperedMagic[0] = 'X'
	if _, err := Open(key, tamperedMagic); !errors.Is(err, domain.ErrDecrypt) {
		t.Fatalf("magic tamper err = %v, want domain.ErrDecrypt", err)
	}
	tamperedBody := bytes.Clone(sealed)
	tamperedBody[len(tamperedBody)-1] ^= 0xff
	if _, err := Open(key, tamperedBody); !errors.Is(err, domain.ErrDecrypt) {
		t.Fatalf("body tamper err = %v, want domain.ErrDecrypt", err)
	}
}
...
func TestOpenWithWrongKeyFails(t *testing.T) {
	...
	if _, err := Open(mustKey(t), sealed); !errors.Is(err, domain.ErrDecrypt) {
...
func TestShortDataFails(t *testing.T) {
	...
	for _, data := range [][]byte{nil, []byte("ENVAULT"), append(Header(), make([]byte, NonceSize)...)} {
		if _, err := Open(key, data); !errors.Is(err, domain.ErrDecrypt) {
			t.Fatalf("Open(%d bytes) err = %v, want domain.ErrDecrypt", len(data), err)
		}
	}
}
...
func TestSealRandomnessFailure(t *testing.T) {
	if _, err := seal(failingReader{}, make([]byte, domain.KeySize), nil); err == nil {
