...
func openEditorSession(t *testing.T, opts Options) *Session {
	...
	s, err := Open(domain.Env{Name: "postgres-local"}, opts)
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
...
func TestEditorTempNameForInvalidEnv(t *testing.T) {
	s, err := Open(domain.Env{Name: "../fora"}, Options{
...
func TestEditorOpenRejectsInvalidEditor(t *testing.T) {
	_, err := Open(domain.Env{Name: "a"}, Options{Lookup: editorLookup(map[string]string{EnvEditor: `"vi`})})
