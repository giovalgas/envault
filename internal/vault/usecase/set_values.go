...
func (uc *SetValues) Execute(ctx context.Context, name string, vars []VarView) (EnvView, error) {
	env, err := modifyEnv(ctx, uc.repo, uc.clock, name, func(env *domain.Env) error {
		for _, v := range vars {
