...
1. **Lista** (tela inicial). O cabeçalho mostra só `envault`. Duas colunas: à esquerda, as envs com nome, número de chaves, última edição e marcador de seleção; à direita, descrição, tags e chaves da env focada, com valores sempre mascarados, cada valor logo depois do nome da chave, na mesma linha, com um separador curto. Um painel fixo abaixo do cabeçalho mostra a seleção global no formato `SELECTED_ENVS=a,b,c`, com os nomes na ordem de marcação separados por vírgula, sem espaço, e `SELECTED_ENVS=` quando não há seleção; cada env marcada leva o número correspondente (`[1]`, `[2]`) na lista, e as demais ficam com `[ ]`. Se o cofre não existir no local resolvido, a TUI o cria antes de listar e mostra no status onde ficou o `vault.enc`; a CLI continua saindo com o código 4 nessa mesma situação.
